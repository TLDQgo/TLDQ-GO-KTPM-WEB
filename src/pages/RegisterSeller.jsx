...
export default function RegisterSeller() {
  ...
  const [form, setForm] = useState({
    ...
    email: user?.email || "",
    phone: user?.phone || "",
    address: user?.customerProfile?.address_line || user?.sellerProfile?.address_line || "",
    password: "",
    confirmPassword: "",
  });
    ...
    try {
      ...
      if (isCustomerLoggedIn) {
        const res = await authApi.upgradeSeller({
          phone: form.phone,
          full_name: form.fullName, // Gửi họ tên riêng
          shop_name: normalizedShopName, // Gửi tên shop riêng
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-gray-600">
              {submitting ? "Đang xử lý..." : "Đã có tài khoản? "}
              <Link
                to="/login-seller"
